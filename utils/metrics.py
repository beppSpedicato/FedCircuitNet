# Copied verbatim from code_examples/CircuitNet/drc_prediction/utils/metrics.py.
# Only change: `build_metric` looks the metric callable up via ``globals()``
# instead of a self-referential ``import utils.metrics as metrics``.
...
def calculated_score(
    ...
    for idx, pred in enumerate(preds):
        ...
        target_probabilities = np.load(
            os.path.join(save_path, "test_result", pred)
        ).reshape(-1)
...
def multi_process_score(out_name=None, threshold=0.0, label_path=None, save_path=None):
    ...
    preds = [
        str(p).split("/")[-1]
        for p in pathlib.Path(save_path, "test_result").rglob("*.npy")
    ]
    ...
    os.system(
        "cp {} {}".format(
            os.path.join(temp_path, f"{out_name}"),
...
def roc_prc(save_path):
    (
        ...
        fp,
        fn,
    ) = calculate_all(os.path.join(os.getcwd(), save_path, "roc_prc.csv"))
    ...
    return (
        ...
        tpr_list_res,
        fpr_list,
        np.mean(precision_sum_List),
        np.mean(accuracy_sum_List),
        np.sum(tp),
        np.sum(tn),
        np.sum(fp),
        np.sum(fn),
